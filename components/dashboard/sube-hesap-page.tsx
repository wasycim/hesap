...
import { getSubeHesapInfo, SUBE_HESAP_INFO, type SubeHesapKey } from "@/lib/sube-utils"

const iconStyles: Record<SubeHesapKey, string> = {
  carsi: "bg-cyan-500/15 text-cyan-600 dark:text-cyan-300",
  darica: "bg-indigo-500/15 text-indigo-600 dark:text-indigo-300",
  onDort: "bg-lime-500/15 text-lime-700 dark:text-lime-300",
  besA: "bg-emerald-500/15 text-emerald-700 dark:text-emerald-300",
}

export function SubeHesapPage({ subeKey }: { subeKey: SubeHesapKey }) {
  ...
  const pageInfo = SUBE_HESAP_INFO[subeKey]
  const isCurrentSube = getSubeHesapInfo(currentSube)?.key === subeKey

  useEffect(() => {
    if (!loading && currentSube && !isCurrentSube) router.replace("/dashboard")
  }, [currentSube, isCurrentSube, loading, router])
  ...
  if (loading || !currentSube || !isCurrentSube) {
    return (
      ...
      <header className="border-b bg-card px-4 py-4 sm:px-6 lg:px-8">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div className="flex items-center gap-3">
            <div className={`grid h-11 w-11 place-items-center rounded-lg ${iconStyles[subeKey]}`}>
