...
const BUILTIN_FIRMALAR: CompanyOption[] = [
...
export default function PerformansAnaliziPage() {
  ...
  const availableCompanies = useMemo(() => {
    const list: CompanyOption[] = [...BUILTIN_FIRMALAR]

    // Group custom firms from database by normalized name
    ...
    dbFirmalar.forEach((firm) => {
      ...
      // Don't add if it's already matching a builtin name
    ...
    // Sort custom companies by branch coverage count descending (most popular custom firms first)
  ...
  const analyticsData = useMemo(() => {
    ...
    return contextSubeler.map((sube) => {
      ...
      const monthMap = new Map<string, { label: string; ciro: number; recordCount: number }>()
      ...
      subeRecords.forEach((record) => {
        ...
        if (val > 0) {
          ...
          if (!Number.isNaN(date.getTime())) {
            ...
            const current = monthMap.get(sortKey) || { label, ciro: 0, recordCount: 0 }
            current.ciro += val
            current.recordCount++
      ...
      return {
        ...
        subeKod: sube.kod,
        totalCiro,
        averageDailyCiro: recordCount > 0 ? totalCiro / recordCount : 0,
        recordCount,
        monthlyBreakdown
      }
    })
  ...
  const kpiData = useMemo(() => {
    ...
    analyticsData.forEach((subeData) => {
      ...
      totalActiveDays += subeData.recordCount
  ...
  const chartsData = useMemo(() => {
    ...
    analyticsData.forEach((subeData) => {
      // If we filtered a specific sube, only include that one
  ...
  return (
    <div className="space-y-6 p-4 sm:p-6 lg:p-8 max-w-[1600px] mx-auto animate-in fade-in duration-500">
      ...
      <Card className="border border-border/60 bg-card/65 backdrop-blur-md shadow-sm overflow-hidden">
        ...
        <CardContent className="p-4">
          {filteredCompanies.length === 0 ? (
            ...
            <div className="space-y-4">
              {/* Ortak (Built-in) Firmalar */}
              {(!searchTerm.trim() || filteredCompanies.some(c => c.type === "builtin")) && (
                <div>
                  <div className="text-xs font-semibold uppercase tracking-wider text-muted-foreground mb-2 flex items-center gap-1.5">
                    ...
                    Ortak Firmalar (Öncelikli)
                  </div>
                  <div className="flex flex-wrap gap-2">
                    {filteredCompanies.filter(c => c.type === "builtin").map((comp) => {
                      ...
                      return (
                        <button
                          ...
                          className={`flex items-center gap-2 px-4 py-2.5 rounded-xl border text-sm transition-all duration-300 relative group cursor-pointer ${
                            isSelected
                              ? "bg-gradient-to-r from-indigo-600 to-purple-600 text-white border-transparent shadow-md scale-105 font-medium"
                              : "bg-background/80 hover:bg-accent border-border/80 hover:scale-[1.02] text-foreground/80"
                          }`}
                        >
                          <span className={`h-2.5 w-2.5 rounded-full ${isSelected ? "bg-white" : comp.color} shrink-0`} />
              ...
              {/* Dinamik Firmalar */}
              {(!searchTerm.trim() || filteredCompanies.some(c => c.type === "custom")) && (
                <div className="border-t pt-3">
                  <div className="text-xs font-semibold uppercase tracking-wider text-muted-foreground mb-2 flex items-center gap-1.5">
                    <span className="h-1.5 w-1.5 rounded-full bg-indigo-500" />
                    Şubeye Özel Diğer Firmalar
                  </div>
                  <div className="flex flex-wrap gap-2">
                    {filteredCompanies.filter(c => c.type === "custom").map((comp) => {
                      const isSelected = selectedCompanyKey === comp.key
                      ...
                      return (
                        ...
                        >
                          <span className={`h-2.5 w-2.5 rounded-full ${isSelected ? "bg-white animate-pulse" : "bg-slate-400 dark:bg-slate-500"} shrink-0`} />
                          <span>{comp.label}</span>
                          <Badge 
                            variant="outline" 
                            className={`text-[9px] px-1 py-0 border-none rounded-md leading-none ${
      ...
      {loading ? (
        ...
        <>
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {/* KPI 1: Toplam Ciro */}
            <Card className="relative overflow-hidden border border-border/60 bg-gradient-to-br from-indigo-50/40 via-card to-card dark:from-indigo-950/10 dark:via-card dark:to-card shadow-sm">
              ...
              <CardContent>
                ...
                <p className="text-[10px] text-muted-foreground mt-1.5 flex items-center gap-1">
                  ...
                  Seçilen tarih aralığındaki genel toplam.
                </p>
            ...
            <Card className="relative overflow-hidden border border-border/60 bg-gradient-to-br from-emerald-50/40 via-card to-card dark:from-emerald-950/10 dark:via-card dark:to-card shadow-sm">
              <CardHeader className="pb-2">
            ...
            <Card className="relative overflow-hidden border border-border/60 bg-gradient-to-br from-amber-50/40 via-card to-card dark:from-amber-950/10 dark:via-card dark:to-card shadow-sm">
              <CardHeader className="pb-2">
              ...
              <CardContent>
                ...
                <p className="text-[10px] text-muted-foreground mt-1.5 flex items-center gap-1">
                  ...
                  Kayıt yapılan günlerin günlük ortalaması.
                </p>
            ...
            {/* KPI 4: Aktif Şubeler */}
            <Card className="relative overflow-hidden border border-border/60 bg-gradient-to-br from-purple-50/40 via-card to-card dark:from-purple-950/10 dark:via-card dark:to-card shadow-sm">
              <CardHeader className="pb-2">
              ...
              <CardContent>
                ...
                <p className="text-[10px] text-muted-foreground mt-1.5 flex items-center gap-1">
                  ...
                  Bu firmaya ait kayıt oluşturan şubeler.
                </p>
          ...
          <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
            ...
            <Card className="border border-border/60 bg-card/75 backdrop-blur-md shadow-sm">
                        ...
                        <PieChart>
                          ...
                          >
                            {chartsData.subesComparison.map((entry, index) => {
                              // Nice distinct colors for each branch
                              const colors = [
          ...
          <Card className="border border-border/60 bg-card/65 backdrop-blur-md shadow-sm">
            <CardHeader className="pb-3 border-b">
              <CardTitle className="text-base font-bold flex items-center gap-2">
              ...
              <CardDescription>
                Şubelerin ciro oranları, günlük ortalamalar ve aylık bazda kırılımları. Satırlara tıklayarak aylık geçmişi inceleyebilirsiniz.
              </CardDescription>
            </CardHeader>
                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="border-b bg-muted/30 text-xs font-semibold text-muted-foreground uppercase">
                      ...
                      <th className="p-4 text-right hidden sm:table-cell">Kayıt Sayısı</th>
                    </tr>
                  </thead>
                                        ...
                                        <table className="w-full text-xs">
                                          <thead>
                                            <tr className="border-b bg-muted/40 font-semibold text-muted-foreground uppercase">
                                              ...
                                              <th className="p-2.5 text-right">Kayıtlı Gün</th>
                                            </tr>
                                          </thead>
