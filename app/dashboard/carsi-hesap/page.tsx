...
import { isCarsiSube } from "@/lib/sube-utils"
...
export default function CarsiHesapPage() {
  ...
  useEffect(() => {
    if (!loading && currentSube && !isCarsi) router.replace("/dashboard")
  }, [currentSube, isCarsi, loading, router])
  ...
  if (loading || !currentSube || !isCarsi) {
    return (
      ...
      <header className="border-b bg-card px-4 py-4 sm:px-6 lg:px-8">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div className="flex items-center gap-3">
            <div className="grid h-11 w-11 place-items-center rounded-lg bg-cyan-500/15 text-cyan-600 dark:text-cyan-300">
              <Landmark className="h-6 w-6" />
            </div>
            <div>
              <h1 className="text-2xl font-bold text-foreground">Çarşı Hesap</h1>
